:: this script is autogenerated by 'ant
            startscripts'
            :: it starts a LAS2peer node providing the service 'de.rwth.dbis.acis.activitytracker.service.ActivityTrackerService' of this project
            :: pls execute it from the bin folder of your deployment by double-clicking on it

            %~d0
            cd %~p0
            cd ..
            set BASE=%CD%
            set CLASSPATH="%BASE%/lib/*;"

            java -cp %CLASSPATH% i5.las2peer.tools.L2pNodeLauncher -p 9011 uploadStartupDirectory('etc/startup') startService('de.rwth.dbis.acis.activitytracker.service.ActivityTrackerService@0.1',  'SampleServicePass') startWebConnector interactive
            pause

        